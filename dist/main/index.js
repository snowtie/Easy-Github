import electron, { app as app$1, safeStorage, ipcMain as ipcMain$1, Menu, BrowserWindow, dialog } from "electron";
import path, { normalize as normalize$1 } from "node:path";
...
function canPersistAccessToken() {
  return safeStorage.isEncryptionAvailable();
}
function saveAccessToken(token) {
  if (!safeStorage.isEncryptionAvailable()) {
    store$2.delete(TOKEN_KEY);
    throw new Error("OS 암호화 저장소를 사용할 수 없어 토큰을 저장할 수 없습니다");
  }
  const encrypted = safeStorage.encryptString(token);
  store$2.set(TOKEN_KEY, encrypted.toString("base64"));
}
function loadAccessToken() {
  const value = store$2.get(TOKEN_KEY);
...
function requestLog(octokit) {
  octokit.hook.wrap("request", (request2, options) => {
    ...
    return request2(options).then((response) => {
      ...
      octokit.log.error(
        `${requestOptions.method} ${path2} - ${error2.status} with id ${requestId} in ${Date.now() - start}ms`
      );
      throw error2;
    });
  });
...
const handler = {
  ...
  get({ octokit, scope: scope2, cache: cache2 }, methodName) {
...
function createOctokit(accessToken) {
  return new Octokit2({ auth: accessToken });
}
class AuthService {
  getStatus() {
  ...
  async setToken(token) {
    ...
    const octokit = createOctokit(trimmed2);
    try {
      const { data } = await octokit.request("/user");
      saveAccessToken(trimmed2);
      return data;
    } catch (err) {
...
function getLearningProgress() {
  return store$1.get("progress") ?? defaultProgress;
}
function updateLearningProgress(partial) {
  const current = getLearningProgress();
  const next2 = { ...current, ...partial };
  store$1.set("progress", next2);
  return next2;
}
function getGuideCompleted() {
  return Boolean(store$1.get("guideCompleted", false));
}
function setGuideCompleted(completed) {
  store$1.set("guideCompleted", completed);
}
...
(function(exports$1) {
  ...
  const debug_12 = __importDefault(srcExports);
...
function filterType(input, filter, def2) {
  if (filter(input)) {
    return input;
  }
  return arguments.length > 2 ? def2 : void 0;
}
...
function versionResponse(major2 = 0, minor2 = 0, patch2 = 0, agent = "", installed = true) {
...
var init_version = __esm({
  "src/lib/tasks/version.ts"() {
    ...
    parsers7 = [
      new LineParser(
        /version (\d+)\.(\d+)\.(\d+)(?:\s*\((.+)\))?/,
        (result, [major2, minor2, patch2, agent = ""]) => {
          Object.assign(
            result,
            versionResponse(asNumber(major2), asNumber(minor2), asNumber(patch2), agent)
          );
        }
      ),
      new LineParser(
        /version (\d+)\.(\d+)\.(\D+)(.+)?$/,
        (result, [major2, minor2, patch2, agent = ""]) => {
          Object.assign(result, versionResponse(asNumber(major2), asNumber(minor2), patch2, agent));
        }
      )
    ];
  }
...
function toPath(input) {
  ...
  return path2 && normalize$1(path2);
}
...
async function ensureCloneTargetIsEmpty(targetPath) {
  try {
    const stat = await fs$1.stat(targetPath);
    if (!stat.isDirectory()) {
      throw new Error("저장 경로가 폴더가 아닙니다");
    }
    const entries = await fs$1.readdir(targetPath);
    if (entries.length > 0) {
...
function mapSimpleGitStatusToType(indexStatus, workingStatus) {
...
async function getGitChanges(repoPath) {
  ...
  const diff2 = await git.diffSummary();
  const statByFile = /* @__PURE__ */ new Map();
  for (const f of diff2.files) {
    statByFile.set(f.file, { insertions: f.insertions, deletions: f.deletions });
  }
  return status.files.map((f) => {
    const type2 = mapSimpleGitStatusToType(f.index, f.working_dir);
...
async function getGitLog(repoPath, maxCount) {
  const git = esm_default(repoPath);
  return await git.log({ maxCount });
}
async function getGitDiff(repoPath, filePath) {
  const git = esm_default(repoPath);
...
async function createBranch(repoPath, branchName, baseBranch) {
  const git = esm_default(repoPath);
...
async function mergeBranch(repoPath, fromBranch) {
  const git = esm_default(repoPath);
  try {
...
async function getGitStatusSummary(repoPath) {
  const git = esm_default(repoPath);
  const status = await git.status();
...
function registerIpcHandlers() {
  ...
  ipcMain$1.handle(IPC_CHANNELS.APP.PING, (event) => {
    if (!validateIpcSender(event)) return "blocked";
    return "pong";
  });
  ipcMain$1.handle(IPC_CHANNELS.APP.OPEN_EXTERNAL, async (event, url) => {
    ...
    await shell2.openExternal(parsed.toString());
  });
  ipcMain$1.handle(IPC_CHANNELS.AUTH.SET_TOKEN, async (event, token) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await authService.setToken(token);
  });
  ipcMain$1.handle(IPC_CHANNELS.AUTH.LOGOUT, async (event) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    await authService.logout();
  });
  ipcMain$1.handle(IPC_CHANNELS.AUTH.GET_TOKEN_STATUS, (event) => {
    if (!validateIpcSender(event)) return { authenticated: false };
    return authService.getStatus();
  });
  ipcMain$1.handle(IPC_CHANNELS.AUTH.GET_USER, async (event) => {
    if (!validateIpcSender(event)) return null;
    return await authService.getUser();
  });
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.GET_MY_PROFILE, async (event) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    const token = loadAccessToken();
  ...
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.LIST_PULLS, async (event, owner, repo) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await listPullRequests(owner, repo);
  });
  ipcMain$1.handle(
    IPC_CHANNELS.GITHUB.REVIEW_PULL,
    async (event, owner, repo, pullNumber, reviewEvent, body) => {
      if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
  ...
  ipcMain$1.handle(
    ...
    async (event, owner, repo, title2, body, head, base) => {
      if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
      return await createPullRequest({ owner, repo, title: title2, body, head, base });
    }
  );
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.MERGE_PULL, async (event, owner, repo, pullNumber) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await mergePullRequest({ owner, repo, pullNumber });
  });
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.LIST_ISSUES, async (event, owner, repo, state) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await listIssues(owner, repo, state);
  });
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.CREATE_ISSUE, async (event, owner, repo, title2, body) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await createIssue({ owner, repo, title: title2, body });
  });
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.CLOSE_ISSUE, async (event, owner, repo, issueNumber) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await closeIssue({ owner, repo, issueNumber });
  });
  ipcMain$1.handle(
    IPC_CHANNELS.GITHUB.COMMENT_ISSUE,
    async (event, owner, repo, issueNumber, body) => {
      if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
      return await commentIssue({ owner, repo, issueNumber, body });
    }
  );
  ipcMain$1.handle(IPC_CHANNELS.GITHUB.LIST_REPOS, async (event) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await listUserRepositories();
  });
  ipcMain$1.handle(
    IPC_CHANNELS.GITHUB.CREATE_REPO,
    async (event, name, description2, isPrivate) => {
      if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
      return await createRepository({ name, description: description2, isPrivate });
    }
  );
  ipcMain$1.handle(IPC_CHANNELS.STORE.GET_LEARNING_PROGRESS, (event) => {
    if (!validateIpcSender(event)) return null;
    return getLearningProgress();
  });
  ipcMain$1.handle(IPC_CHANNELS.STORE.UPDATE_LEARNING_PROGRESS, (event, partial) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return updateLearningProgress(partial);
  });
  ipcMain$1.handle(IPC_CHANNELS.STORE.GET_GUIDE_COMPLETED, (event) => {
    if (!validateIpcSender(event)) return false;
    return getGuideCompleted();
  });
  ipcMain$1.handle(IPC_CHANNELS.STORE.SET_GUIDE_COMPLETED, (event, completed) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    setGuideCompleted(completed);
  });
  ipcMain$1.handle(IPC_CHANNELS.STORE.GET_PROJECTS, (event) => {
    if (!validateIpcSender(event)) return [];
    return getProjects();
  });
  ipcMain$1.handle(IPC_CHANNELS.STORE.SAVE_PROJECTS, (event, projects) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    saveProjects(projects);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.CLONE, async (event, repoUrl, targetPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    await cloneRepository(repoUrl, targetPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.STATUS, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await getGitStatusSummary(repoPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.FETCH, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    await fetchRepository(repoPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.PULL, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await pullRepository(repoPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.PUSH, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await pushRepository(repoPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.CHANGES, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await getGitChanges(repoPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.STAGE, async (event, repoPath, files) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    await stageFiles(repoPath, files);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.UNSTAGE, async (event, repoPath, files) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    await unstageFiles(repoPath, files);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.COMMIT, async (event, repoPath, message) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await gitCommit(repoPath, message);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.LOG, async (event, repoPath, maxCount) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await getGitLog(repoPath, maxCount);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.DIFF, async (event, repoPath, filePath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await getGitDiff(repoPath, filePath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.BRANCH_LIST, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await listLocalBranches(repoPath);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.BRANCH_CHECKOUT, async (event, repoPath, branchName) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
  ...
  ipcMain$1.handle(IPC_CHANNELS.GIT.BRANCH_DELETE, async (event, repoPath, branchName) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    await deleteBranch(repoPath, branchName);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.MERGE, async (event, repoPath, fromBranch) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
    return await mergeBranch(repoPath, fromBranch);
  });
  ipcMain$1.handle(IPC_CHANNELS.GIT.ORIGIN_URL, async (event, repoPath) => {
    if (!validateIpcSender(event)) throw new Error("IPC sender not allowed");
...
URL: ${validatedURL}`);
  ...
  if (!app$1.isPackaged && process.env["ELECTRON_RENDERER_URL"]) {
